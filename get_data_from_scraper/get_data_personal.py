...
def get_simple_summary(scrape):
    ...
    try:
        for username in scrape.usernames:
            ...
            user_dataframe = pd.DataFrame(columns=['DATE', 'MEDIA', 'LIKES', 'COMMENTS'])

            print(username)
            it = scrape.query_media_gen(user)
            for i, item in tqdm.tqdm(enumerate(it)):
                date = datetime.datetime.fromtimestamp(
                    item['taken_at_timestamp'] - 13 * 3600).strftime('%Y-%m-%d %H:%M:%S')
                media = type_of_media[item['is_video']]
                likes = item['edge_media_preview_like']['count']
                comments = item['edge_media_to_comment']['count']
                likes_comments_df = pd.DataFrame([[date, media, likes, comments]],
                                                 columns=['DATE', 'MEDIA', 'LIKES', 'COMMENTS'])
                user_dataframe = user_dataframe.append(likes_comments_df, ignore_index=True)

                # if not item['is_video']:
                #     url =sdz item['thumbnail_resources'][1]['src']
                #     r = requests.get(url)
                #
                #     try:
